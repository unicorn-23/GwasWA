...
class Enformer(snt.Module):
        ...
        transformer = Sequential(lambda: [
            Sequential(lambda: [
                ...
                Residual(Sequential(lambda: [
                    snt.LayerNorm(axis=-1, create_scale=True,
                                  create_offset=True, scale_init=snt.initializers.Ones()),
    ...
    def __call__(self, inputs: tf.Tensor, is_training: bool) -> Dict[str, tf.Tensor]:
        ...
        return {
            head: head_module(trunk_embedding, is_training=is_training) for head, head_module in self.heads.items()
        }
